...
def draw_chain_diff(old_data, new_data, target, format, extension=None):
    ...
    for component in components:
        ...
        if len(sg) <= 2:
            ...
            this_component_pos = {n: np.array((i, 0), dtype=np.float64) for i, n in enumerate(sg)}
        else:
            # shortest weighted undirected path in the component
            dist = dict(nx.shortest_path_length(nx.classes.graphviews.generic_graph_view(sg, nx.Graph), weight="layout_weight"))
            print(dist)
        ...
        if edges:
            ...
            scale = ideal_gap / min(np.linalg.norm(this_component_pos[dest] - this_component_pos[source]) for source, dest in edges)
            ...
            if np.any(np.greater(old_size * scale, max_size)):
                logger.warning("Component scale %r too high (range %r, max %r)", scale, old_size, max_size)
            ...
            del distances
            vecs = np.array(np.triu_indices(this_component_pos_v.shape[0], k=1))[:, furthest_index]
    ...
    component_off = dict(zip(component_pos, pack_components(component_pos_v.values(), ideal_gap)))
    ...
    min_x = min(map(operator.itemgetter(0), pos.values()))
    max_x = max(map(operator.itemgetter(0), pos.values()))
    ptp_x = max_x - min_x
    min_y = min(map(operator.itemgetter(1), pos.values()))
    ...
    fig.savefig(data, dpi=dpi, bbox_inches=bbox, format=format, pad=0)
    data.seek(0)
    return data
...
def pad(x, y):
    if x and y:  # they're >=1, so we can subtract 1
        ...
        return {(x-1, y), (x, y), (x+1, y), (x-1, y+1), (x, y+1), (x+1, y+1)}
    elif y:
        return {(x, y-1), (x+1, y-1), (x, y), (x+1, y), (x, y+1), (x+1, y+1)}
    else:
...
if __debug__:
    def render_grid(grid, results, component_pos, padding):
        ...
        filled = {conv(result[0] + x, result[1] + y, padding) for result, nodes in zip(results, component_pos) for x, y in nodes if result}
        ret = [str(filled)]
        for i, row in enumerate(grid):
            ret.append(" ".join("🞓" if (j, i) in filled else "🞔" if row & 1 << j else "🞎" for j in range(length)))
        return "\n".join(ret)
...
def pack_components(component_pos, padding):
    ...
    for component, nodes in sorted(enumerate(component_pos), key=lambda x: len(x[1]), reverse=True):
        ...
        for i, j in coord_iter():
            if __debug__:
                if debug_counter == 1000:
                    logger.warning("Very large grid iteration detected. State: i,j=%r,%r, component=%r, counter=%r, converted=%r, grid=%r, results=%r, component_pos=%r, padding=%r", i, j, component, debug_counter, converted, grid, results, component_pos, padding)
                    debug_counter = 0
                debug_counter += 1
                    ...
                    if __debug__:
                        try:
                            ...
                            if (y + j) - len(grid) >= 1000:
                                logger.warning("Very large grid expansion detected. State: grid=%r, x,y=%r,%r", grid, x + i, y + j)
        ...
        if __debug__:
            logger.debug(render_grid(grid, results, component_pos, padding))
    return results
