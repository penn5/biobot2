...
class User:
    ...
    def __post_init__(self):
        assert isinstance(self.id, (int, types.NoneType)), self.id
        assert isinstance(self.usernames, tuple), self.usernames
        assert isinstance(self.deleted, (bool, types.NoneType)), self.deleted

    @property
    def key(self):
        return get_key(self.id, self.usernames)
...
class FullUser:
    ...
    def __post_init__(self):
        assert isinstance(self.user, User), self.user
        assert isinstance(self.about, str), self.about

    @property
    def points_to(self):
        return get_bio_links(self.about)
...
def get_bio_links(about):
    return [x.group()[1:] for x in USERNAME_REGEX.finditer(about)]


def node_to_user(node):
    return FullUser(
        User(node["uid"], node["usernames"], node["deleted"]), node["about"]
    )


def get_key(uid, usernames):
    return (
